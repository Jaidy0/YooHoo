                ...
                script {
                    docker.withRegistry('https://index.docker.io/v1/', "${DOCKER_HUB_CREDENTIALS_ID}") {
                        sh """
                            ...
                            docker tag ${BACKEND_IMAGE}:${CANARY_TAG} ${BACKEND_IMAGE}:stable-latest
                            docker tag ${FRONTEND_IMAGE}:${CANARY_TAG} ${FRONTEND_IMAGE}:${STABLE_TAG}
                            docker tag ${FRONTEND_IMAGE}:${CANARY_TAG} ${FRONTEND_IMAGE}:stable-latest

                            docker push ${BACKEND_IMAGE}:${STABLE_TAG}
                            docker push ${BACKEND_IMAGE}:stable-latest
                            docker push ${FRONTEND_IMAGE}:${STABLE_TAG}
                            docker push ${FRONTEND_IMAGE}:stable-latest
                        """
                    }
                    parallel(
                        "Backend Promotion": {
